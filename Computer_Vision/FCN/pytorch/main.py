...
# --- model save
model_save_path = f'./model/FCN{num_model}s/'
image_save_path = f'./seg_result/FCN{num_model}s/'
saving = True
saving_point = 10000
if not saving:
    saving_point = 0
...
if num_model == 32:
    model = FCN_32S(num_cls).to(DEVICE)
elif num_model == 16:
    model = FCN_16S(num_cls).to(DEVICE)
elif num_model == 8:
    model = FCN_8S(num_cls).to(DEVICE)
else:
    import sys
